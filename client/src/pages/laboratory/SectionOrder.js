...
export default function SectionOrder() {
  ...
  const fetchOrder = async () => {
    ...
    try {
      await axios.get(`${apiUrl}/order/section-orders/${orderid}/${section}`).then((response) => {
    ...
    // Fetch order Notes
    try {
      await axios.get(`${apiUrl}/order/get-order-notes/${orderid}`).then((response) => {
    ...
    // Fetch users
    try {
      await axios.get(`${apiUrl}/auth/get-all-users`).then((response) => {
        // Sort users by position
        const rmtUsers = response.data.filter(
          (user) => user.position === 'Registered Medical Technologist'
        );
  ...
  const fetchCommentList = async () => {
    try {
      await axios.get(`${apiUrl}/test/comments/fetch-all`).then((response) => {
  ...
  const handleResult = async (e, id, index) => {
    setOrder((prevOrder) => {
    ...
    try {
      axios
        .put(`${apiUrl}/test/update-test-result/`, { id: id, result: e.target.value })
        .then((res) => {
  ...
  const updateComment = async (index, testid, newComment) => {
    ...
    try {
      axios
        .put(`${apiUrl}/test/update-test-comment/`, { id: testid, comment: newComment })
        .then((res) => {
          if (res.data.errormessage) {
            alert(res.data.errormessage);
          } else {
  ...
  const releaseResult = async () => {
    ...
    const incompleteResults = order.tests.some(
      (test) => test.test.package !== true && (!test.result || test.result.trim() === '')
    );
    ...
    if (missingFields.length > 0) {
      alert(
        `Please fill in the following fields before releasing the result:\n- ${missingFields.join(
          '\n- '
        )}`
      );
      return;
    ...
    axios
      .put(`${apiUrl}/order/edit-order/${orderid}/Released`, data)
      .then((res) => {
        if (res.data.errormessage) {
          alert(res.data.errormessage);
        } else {
  ...
  const undoRelease = async () => {
    ...
    axios
      .put(`${apiUrl}/order/edit-order/${orderid}/Pending`, data)
      .then((res) => {
        if (res.data.errormessage) {
          alert(res.data.errormessage);
        } else {
      ...
      {order && (
        <>
          ...
          <div className="mb-4">
            ...
            <p className="p-0 m-0">
              <strong>Lab Number:</strong> {order.labnumber.labnumber}
            </p>
          ...
          <div className="d-inline-block col-md-6">
            ...
            <p className="p-0 m-0">
              ...
              {moment(order.labnumber.createdAt).format('MMMM DD YYYY, h:mm a')}
            </p>
            <p className="p-0 m-0">
              <strong>Requesting Physician:</strong> {order.labnumber.requesting_physician}
            </p>
            <p className="p-0 m-0">
              <strong>Patient Type:</strong> {order.labnumber.patient_type}
            </p>
          </div>

          <div className="d-inline-block col-md-12 mt-3 mb-3">
            <h4 className="p-0 m-0 fw-bold text-decoration-underline mb-2">
                ...
                order.tests.map((test, index) =>
                  test.test.package === true ? (
                    ...
                    <tr key={index}>
                      ...
                      {test.test.options === '' || test.test.options === 'None' ? (
                        <td>
                          <input
                      ...
                      <td className="text-center">
                        {test.test_comment ? (
                          <Button
                            onClick={() => {
                              handleModal(index, test._id, test.test.name);
                            }}
          ...
          {testSelected && (
            <Commentsmodal
              handleCommentListModal={handleCommentListModal}
          ...
          <CommentListModal
            ...
            setCommentListModal={setCommentListModal}
          ...
          <div>
            <div className="d-flex justify-content-between mb-3">
              <div className="inline block col-md-3">
                <label className="text-decoration-underline fw-bold" htmlFor="global-comments">
                  Global Comment/s:
                </label>
                <br />
                <textarea
                  ...
                  defaultValue={order.global_comments || ''}
                  style={{ resize: 'none' }}
              ...
              <div className="inline-block col-md-3">
                <label className="fw-bold text-decoration-underline" htmlFor="performer">
                  Performer:
                </label>
                <br />
                <select
                  ...
                  id="performer"
                >
                  <option defaultValue={order.performed_by || ''} value="">
                    Select Performer
                  </option>
                  {users.rmt.length > 0 &&
              ...
              <div className="inline-block col-md-3">
                <label className="text-decoration-underline fw-bold" htmlFor="pathologist">
                  Pathologist:
                </label>
                <br />
                <select
                  ...
                  id="pathologist"
                >
                  <option defaultValue={order.pathologist || ''} value="">
                    Select Pathologist
                  </option>
                  {users.pathologists &&
                    users.pathologists.length > 0 &&
            ...
            {order.status === 'Pending' ? (
              ...
              <Button onClick={undoRelease} className="border border-black mx-1" variant="danger">
                Undo Release Result
              </Button>
            )}
          </div>
