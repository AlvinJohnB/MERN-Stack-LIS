...
export default function ResultsIndex() {
  ...
  const fetchPtData = async () => {
    try{
        setIsLoading(true);
        await axios.get(`${apiUrl}/order/${id}`)
        .then((response) => {
...
const fetchSectionOrders = async () => {
    try{
        setIsLoading(true);
        await axios.get(`${apiUrl}/order/s-order/${id}`)
        .then((response) => {
            if(response.data.errormessage) {
                alert(response.data.errormessage);
            } else {
  ...
  return (
    <div className="container-fluid mt-4">
        <h3 align='center' className='text-decoration-underline fw-bold'>Results</h3>

               {/* header */}
                <div className='mb-4'>
                  {/* <h4 className= 'p-0 m-0'>Section: {order.section}</h4> */}
                  <p className= 'p-0 m-0'><strong>Lab Number:</strong> {order.labnumber}</p>
                ...
                <div className='d-inline-block col-md-6 mb-5'>
                    ...
                    <p className='p-0 m-0'><strong>Order Date:</strong> {moment(order.createdAt).format('MMMM DD YYYY, h:mm a')}</p>
                    <p className='p-0 m-0'><strong>Requesting Physician:</strong> {order.requesting_physician}</p>
                    <p className='p-0 m-0'><strong>Patient Type:</strong> {order.patient_type}</p>
                </div>
                ...
                <Accordion defaultActiveKey={[0]} alwaysOpen className='mb-4'>
                    {sectionOrders.map((order, index) => (
                              ...
                              test.test.package === true ? (
                                ...
                                <tr key={index}>
                                  ...
                                  {test.test.options === '' || 'None' ?
                                    (<td>
                                      <input disabled={true} type='text' className='form-control m-0 p-1 text-center' defaultValue={test?.result === '' ? 'PENDING' : test.result} />
                                    </td>)
                                    :
                                    (<td>
                      ...
                      <Button variant={order.status === 'Released' ? 'success' : 'danger'} size={'sm'} disabled={order.status === 'Released' ? false : true}>Generate Result</Button>
                        </Accordion.Body>
                    </Accordion.Item>
                    ))}
                </Accordion>

                <Button variant={order.status === 'Released' ? 'success' : 'danger'} size={'sm'} disabled={order.status === 'Released' ? false : true}>Generate Full Results</Button>
