  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (tests.length !== testsRequested.length) {
        return res
          .status(400)
          .json({ message: 'Some products are invalid or not found.' });
      }
      ...
      const lastOrder = await Model.OrderModel.findOne({
      ...
      const computeSectionTotals = (testsRequested, isDiscounted) => {
        ...
        testsRequested.forEach((test) => {
          const price =
            isDiscounted === true ? test.discounted_price : test.price;
      ...
      const groupTestsBySection = async (testsRequested) => {
        ...
        for (const test of testsRequested) {
          ...
          if (test.package) {
            ...
            const profile = await Model.PackageModel.findOne({
              name: test.testcode,
            }).populate('tests.test');
      ...
      const newOrder = new Model.OrderModel({
        ...
        isDiscounted: isDisc,
        total: Object.values(sectionTotals).reduce(
          (sum, value) => sum + value,
          0
        ), // Total cost
        encoded_by: encoder,
...
OrderRouter.get('/section-orders/:section', async (req, res) => {
  ...
  try {
    ...
    const orders = await Model.SectionOrderModel.find({ section })
      ...
      .populate(
        'tests.test',
        'testcode testname show options unit reference_value_male reference_value_female'
...
// fetch section orders by id
OrderRouter.get(
  '/section-orders/:section_orderid/:section',
  async (req, res) => {
    ...
    try {
      // Fetch all orders and optionally populate related fields
      const orders = await Model.SectionOrderModel.findOne({
        _id: section_orderid,
        section: section,
      })
        .populate('tests') // Populate test details
        .populate(
          'tests.test',
          'testcode name show package options unit reference_value_male reference_value_female'
        ) // Populate test details
